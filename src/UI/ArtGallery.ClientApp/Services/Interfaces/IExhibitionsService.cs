...
{
    Task<ICollection<ExhibitionDto>> GetAllExhibitionsAsync(Guid? museumId = null,
        string apiVersion = Constants.Const.DefaultApiVersion);
    ...
    Task<ExhibitionDto> CreateExhibitionAsync(CreateExhibitionCommand command,
        string apiVersion = Constants.Const.DefaultApiVersion);

    Task<ExhibitionDto> UpdateExhibitionAsync(Guid id, UpdateExhibitionCommand command,
        string apiVersion = Constants.Const.DefaultApiVersion);

    Task<bool> DeleteExhibitionAsync(Guid id, string apiVersion = Constants.Const.DefaultApiVersion);

    Task<bool> AddPaintingToExhibitionAsync(Guid exhibitionId, Guid paintingId,
        string apiVersion = Constants.Const.DefaultApiVersion);

    Task<bool> RemovePaintingFromExhibitionAsync(Guid exhibitionId, Guid paintingId,
        string apiVersion = Constants.Const.DefaultApiVersion);
}
